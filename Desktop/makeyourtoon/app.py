...
def get_binary_file_downloader_html(bin_file, file_label='File'):
    ...
    href = f'<a href="data:application/octet-stream;base64,{bin_str}" download="{os.path.basename(bin_file)}" style="margin:10px 10px;text-decoration:none;font-weight:bold;background-color: DodgerBlue;border: none;color: white;padding: 12px 45px;cursor: pointer;font-size: 20px;" >Download {file_label}</a>'
    return href
...
def main():
    ...
    activities = ['Home','Cartoonize Image','Cartoonize Video','Cartoonize GIF','Image Upscaling','Image Denoise','Resize','Rotate','Crop','About']
    ...
    if sidebar_choice == 'Home':
    	...
    	img,cap = st.beta_columns((1,2))
    	with img:
    		st.image("static/mukesh.jpg",use_column_width=True)
    	with cap:
    		html = '''
    		<div style="margin:auto;padding:20px;">
    		<h2 style="font-size:30px;color:red;font-weight:bold;text-decoration: underline">Mukesh Parmar</h2>
    		<br><br>
    		<h4 style="padding:5px">Xavier Institute of Engineering</h4>
    		<h4 style="padding:5px;">Email : <a href="mailto:20170339.mukeshpnb@student.xavier.ac.in">20170339.mukeshpnb@student.xavier.ac.in</a></h4>
    		</div>
    		'''
    		st.markdown(html,unsafe_allow_html=True)
    	...
    	img,cap = st.beta_columns((1,2))
    	with img:
    		st.image("static/vedant.jpg",use_column_width=True)
    	with cap:
    		html = '''
    		<div style="margin:auto;padding:20px;">
    		<h2 style="font-size:30px;color:red;font-weight:bold;text-decoration: underline">Vedant Prabhu</h2>
    		<br><br>
    		<h4 style="padding:5px">Xavier Institute of Engineering</h4>
    		<h4 style="padding:5px;">Email : <a href="mailto:20170345.vedantpnn@student.xavier.ac.in">20170345.vedantpnn@student.xavier.ac.in</a></h4>
    		</div>
    		'''
    		st.markdown(html,unsafe_allow_html=True)
    	...
    	img,cap = st.beta_columns((1,2))
    	with img:
    		st.image("static/omkar.jpg",use_column_width=True)
    	with cap:
    		html = '''
    		<div style="margin:auto;padding:20px;">
    		<h2 style="font-size:30px;color:red;font-weight:bold;text-decoration: underline">Omkar Gorhe</h2>
    		<br><br>
    		<h4 style="padding:5px">Xavier Institute of Engineering</h4>
    		<h4 style="padding:5px;">Email : <a href="mailto:20170313.omkargpp@student.xavier.ac.in">20170313.omkargpp@student.xavier.ac.in</a></h4>
    		</div>
    		'''
    		st.markdown(html,unsafe_allow_html=True)
	    	...
	    	if st.button("Click Here To Cartoonize Your Video"):
		    	...
		    	with st.spinner("Cartoonizing in progres...."):
			    	...
			    	if flag==0:
				    	...
				    	st.markdown(get_binary_file_downloader_html(video_file.name), unsafe_allow_html=True)
    ...
    elif sidebar_choice == 'Image Upscaling':
    	...
    	if img_file is None:
	    	...
	    	main_img_cv = loadim(img_file)
	    	st.image(main_img_cv,channels = 'BGR',use_column_width = True)
	    	upscale_type = st.selectbox("Select upscaling Method",['EDSR','FSRCNN','Bilinear Interpolation'])
	    	if upscale_type == 'EDSR':
	    		st.info('The Enhanced deep residual Upscaling method is based on a larger model and will take anywhere from 2 - 8 mins to complete upscaling.')
	    		if st.button("Click Here To Upscale"):
	    			if main_img_cv.shape[0] > 1080 or main_img_cv.shape[1] > 1080:
	    				st.error('Image to Large for upscaling. This image is already above 1080 pixels wide')
	    			else:
	    				with st.spinner("Upscaling... This may take long."):
	    					upscaled_image_cv = upScaleEDSR(main_img_cv)
	    					st.success("Image has been upscaled with EDSR")
	    					st.image(upscaled_image_cv, channels="BGR")
	    					link = save('EDSR_',upscaled_image_cv)
		    				st.markdown(get_binary_file_downloader_html(link), unsafe_allow_html=True)
	                			

	    	elif upscale_type == 'Bilinear Interpolation':
	    		...
	    		if st.button("Click Here To Upscale"):
	    			...
	    			st.image(upscaled_image_cv, channels="BGR")
	    			link = save('Bilinear_',upscaled_image_cv)
	    			st.markdown(get_binary_file_downloader_html(link), unsafe_allow_html=True)
	    		...
	    		if st.button("Click Here To Upscale"):
	    			if main_img_cv.shape[0] > 1080 or main_img_cv.shape[1] > 1080:
	    				...
	    				with st.spinner("Upscaling... This may take long."):
	    					...
	    					st.success("Image has been upscaled with FSRCNN")
	    					st.image(upscaled_image_cv, channels="BGR")
	    				link = save('FSRCNN_',upscaled_image_cv)
	    				st.markdown(get_binary_file_downloader_html(link), unsafe_allow_html=True)
		    	
            

    elif sidebar_choice == 'Image Denoise':
    	...
    	if img_file is None:
	    	...
	    	if st.button('Click Here To Resize Your Image'):
	    		dim = dim.split(',')
	    		if len(dim)!=2:
	    			...
	    			link = save("Resize_",resize_image)
	    			st.markdown(get_binary_file_downloader_html(link), unsafe_allow_html=True)
    	...
    	if img_file is None:
	    	...
	    	if st.button('Click Here To Rotate'):
	    		...
	    		link = save('Rotate_',rotated_image)
	    		st.markdown(get_binary_file_downloader_html(link), unsafe_allow_html=True)
    	...
    	if img_file is None:
	    	...
	    	if st.button('Click Here To Crop'):
	    		x = x.split(",")
	    		x = (int(x[0]),int(x[1]))
	    		y = y.split(',')
	    		y = (int(y[0]),int(y[1]))
    	...
    	<li class='font'>Provides other image features like image Upscaling, denoising, rotate and crop</li>
    	</ul>
    	</div>
    	'''
    		st.markdown(html,unsafe_allow_html=True)

    	with st.beta_expander("Contact Developers"):
    		st.markdown("<a href='mailto:20170339.mukeshpnb@student.xavier.ac.in' target='_blank'> Click Here To Mail Mukesh Parmar </a>",unsafe_allow_html=True)
    		st.markdown("<a href='mailto:20170345.vedantpnn@student.xavier.ac.in' target='_blank'> Click Here To Mail Vedant Prabhu </a>",unsafe_allow_html=True)
    		st.markdown("<a href='mailto:20170339.omkargpp@student.xavier.ac.in' target='_blank'> Click Here To Mail Omkar Gorhe </a>",unsafe_allow_html=True)
    	...
    	with st.beta_expander("Technical Paper for this project"):
    		st.markdown("<a href='http://www.ijaresm.com/uploaded_files/document_file/Prof._MartinaRodrigues,_Mukesh_Parmar,_Omkar_Gorhe,_Vedant_Prabhu_tz2T.pdf' target='_blank'> Cartoonization of Images And Videos Using GAN and OPENCV </a>",unsafe_allow_html=True)

    	with st.beta_expander("The Image Upscaling is done based on follow papers"):
